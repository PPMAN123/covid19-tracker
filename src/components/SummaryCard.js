...
const SummaryCard = ({ amount, secondaryAmount, title, description }) => {
    ...
    useEffect(() => {
        if (amount) {
            const amountString = `${new Intl.NumberFormat().format(amount)}`
            if(amountItems.length > 0 && amountItems.length == amountString.length){
                const secondaryAmountString = `${new Intl.NumberFormat('en-US', {signDisplay:'always'}).format(secondaryAmount || 0)}`
    ...
    useEffect(() => {
        if (secondaryAmount) {
            const secondaryAmountString = `${new Intl.NumberFormat('en-US', {signDisplay:'always'}).format(secondaryAmount || 0)}`
            if (secondaryAmountItems.length > 0 && secondaryAmountItems.length < secondaryAmountString.length){
                setTimeout(() => {
    ...
    const secondaryTransitions = useTransition(secondaryAmountItems, item => item.key, {
        from: { transform: 'translate3d(0,-40px,0)' , opacity: 0},
        enter: { transform: 'translate3d(0,0px,0)' , opacity: 1 },
        leave: { transform: 'translate3d(0,-40px,0)' , opacity: 0},
    })
